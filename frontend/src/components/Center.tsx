...
const Center: React.FC = () => {
  ...
  const [voteType, setVoteType] = useState<"MULTI" | "SIMPLE" | null>(null);
  ...
  const handleVoteCreated = (newVoteUuid: string, type: "MULTI" | "SIMPLE") => {
    setVoteUuid(newVoteUuid);
    setVoteType(type);
  ...
  return (
    <div>
      <div className="flex h-[92vh] mt-[5vh] mb-[5vh] rounded-lg overflow-hidden">
        ...
        <div className="flex-1 flex flex-col border border-gray-400 rounded-lg">
          ...
          <div className="bg-gray-50 p-2 border-b-2 border-gray-400 rounded-t-lg">
            ...
            {showAlert && (
              <div className="mt-2 text-center text-sm text-red-600">📢 Alert 활성화됨!</div>
            )}
          </div>
          ...
          <div className="flex flex-1">
            ...
            {/* Center Content */}
            <div className="flex-1 p-4 rounded-lg"></div>
          </div>
        </div>
      ...
      {activeTab === "Vote" ? (
        ...
        ) : voteUuid && voteType === "SIMPLE" ? (
          <SimpleVote voteUuid={voteUuid} />
        ) : (
          <p className="text-gray-700">투표를 생성하려면 Vote 버튼을 클릭하세요.</p>
        )
      ) : (
